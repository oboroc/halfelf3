...
HANDLE WINAPI _export OpenFilePlugin(char *name, const unsigned char *data, int dataSize)
{
  ...
  if(memcmp(data + 8, "ZIP", 3) ||
     !memcmp(data, "********", 8)) return INVALID_HANDLE_VALUE;

  if(!isValidZippedFile( *(ZipedFileHdr*) (data + sizeof(HoHdr)) ))
...
HANDLE WINAPI _export OpenPlugin(int OpenFrom, int Item)
{
  ...
  BYTE data[sizeof(HoHdr) + sizeof(ZipedFileHdr) + 1];
