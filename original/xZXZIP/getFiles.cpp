...
{
  int fSize = fileSize(files[fNum]);
  
  int packedSize = files[fNum].packedSize;
  BYTE* buf = new BYTE[256*files[fNum].noSecs];
  BYTE* pBuf = new BYTE[packedSize];

  if(!setPointer(pcFiles[fNum].offset)) goto error;
  if(!read(pBuf, packedSize)) goto error;
  
  switch(files[fNum].pMethod)
  {
    case 0:
      CopyMemory(buf, pBuf, fSize);
      break;

    case 1:
      unlz(pBuf, packedSize, buf, fSize);
      break;

    case 2:
      unshrink(pBuf, packedSize, buf, fSize);
      break;

    case 3:
      explode(pBuf, packedSize, buf, fSize, fSize < 0x1600, files[fNum].flags & 1);
      break;
  }

  if(crc32(buf, fSize) == files[fNum].crc)
  {
    CopyMemory(buffer, buf, size==0?fSize:size);
    delete[] buf;
    delete[] pBuf;
    return true;
  }
                      ...
                      int opMode)
{
  ...
  char msg[100];
  if(noItems == 1)
    wsprintf(msg, getMsg(MExtract1), panelItem[0].FindData.cFileName);
  else
    wsprintf(msg, getMsg(MExtract2), noItems);

  char historyName[] = "xZXZIP_copy_path";
  
  InitDialogItem items[]={
    DI_DOUBLEBOX,3,1,72,11,0,0,0,0, (char*)MExtract,
    ...
    DI_BUTTON,0,10,0,0,0,0,DIF_CENTERGROUP,1,(char*)MExtractButton,
